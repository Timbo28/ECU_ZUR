...
#define DEVICE_NAME "mcp251x_zhaw"

//---------------------------------------------------------------------------------------------------- modified: thread struct add
static struct task_struct *kthread_poll_1;
static struct task_struct *kthread_poll_2;
static bool kthread_enable = false;
static int kthread_count = 0;
//---------------------------------------------------------------------------------------------------- end
...
static int mcp251x_hw_wake(struct spi_device *spi)
{
	...
	//disable_irq(spi->irq); // modified: deleted
	kthread_enable = false; // modified: added
	...
	//enable_irq(spi->irq); // modified: deleted
	kthread_enable = true; // modified: added
...
static int mcp251x_stop(struct net_device *net)
{
	...
	//---------------------------------------------------------------------------------------------------- modified: close thread added
	if(kthread_count == 1){
		pr_info(KERN_INFO "Cleaning up kthread 1\n");
		kthread_stop(kthread_poll_1);
		pr_info(KERN_INFO "kthread 1 stopped\n");
	}
	else if(kthread_count == 2){
		pr_info(KERN_INFO "Cleaning up kthread 2\n");
		kthread_stop(kthread_poll_2);
		pr_info(KERN_INFO "kthread 2 stopped\n");
	}
	kthread_count--;
	//---------------------------------------------------------------------------------------------------- end
...
//---------------------------------------------------------------------------------------------------- modified: thread (irq) added
static int thread_fn_1(void *dev_id)
{
    struct mcp251x_priv *priv = dev_id;
	struct spi_device *spi = priv->spi;
	struct net_device *net = priv->net;

	pr_info("Info: Thread 1 entered\n");
    while (!kthread_should_stop())
    {
		if (kthread_enable)
		{
			//pr_info("Info: Thread polled\n");
			mutex_lock(&priv->mcp_lock);
			while (!priv->force_quit) {
				//enum can_state new_state;
				u8 intf, eflag;
...
static int thread_fn_2(void *dev_id)
{
    struct mcp251x_priv *priv = dev_id;
	struct spi_device *spi = priv->spi;
	struct net_device *net = priv->net;

	pr_info("Info: Thread 2 entered\n");
    while (!kthread_should_stop())
    {
		if (kthread_enable)
		{
...
static int mcp251x_open(struct net_device *net)
{
	...
	//---------------------------------------------------------------------------------------------------- modified: init thread added
	disable_irq(spi->irq);

	kthread_count++;

	if(kthread_count == 1){
		kthread_poll_1 = kthread_run(thread_fn_1, priv, "mcp251x_poll_thread_1");
		if (IS_ERR(kthread_poll_1))
		{
			pr_err("Error: kthread_run() failed\n");
			goto out_close;
		}
	}
	else if(kthread_count == 2){
		kthread_poll_2 = kthread_run(thread_fn_2, priv, "mcp251x_poll_thread_2");
		if (IS_ERR(kthread_poll_2))
		{
			pr_err("Error: kthread_run() failed\n");
			goto out_close;
		}
